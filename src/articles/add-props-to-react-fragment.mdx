---
title: Add Props to a React Fragment
description: Here's how to add props to a React Fragment
...
```js numbered
const gridData = data.map(({id, firstName, lastName, team}) => {
  return (
    <>
      <span>{firstName}</span>
      <span>{lastName}</span>
...
const gridData = data.map(({id, firstName, lastName, team}) => {
  return (
    <>
      <span key={`${id}:firstName`}>{player.firstName}</span>
      <span key={`${id}:lastName`}>{player.lastName}</span>
      <span key={`${id}:team`}>{player.team}</span>
    </>
  )
})
```
