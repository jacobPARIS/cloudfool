...
class DataLoader {
  ...
  async load(id) {
    if (this.cachedPromises[id]) {
      return this.cachedPromises[id]
    } else {
      this.cachedPromises[id] = null
    }

    await this.schedulingFn()

    if (!this.query) {
      const keys = Object.keys(this.cachedPromises)

      this.query = this.batchFn(keys)
    }

    this.promises[id] = this.query
