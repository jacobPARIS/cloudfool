...
environment variables, and uses a few different tasks to get it running.

The task that runs `sudo -E tailscale up` will run in the background, but unauthenticated users will be prompted to log in. If you put this one at the bottom of the `tasks` list, it will prompt the developer to log in. That only has to happen once, and it saves the Tailscale login token into Gitpod's environment variables to skip the login next time.

The task that runs `sudo tailscaled` will run in the background and doesn't need any developer interaction, so putting it at the top of the `tasks` list will keep it out of the developer's way.
