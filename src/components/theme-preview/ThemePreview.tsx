...
export default function ThemePreview({
  ...
  return (
    <div className="w-full rounded shadow-md">
      ...
      >
        <h3 className="flex items-center justify-center py-1 m-0 mx-auto font-500">
          {themeName}
        </h3>
      </div>
      <div
      ...
      >
        <div className="absolute inset-0 flex overflow-hidden ">
          ...
          <div className="flex flex-col flex-grow">
            ...
            >
              <button
                className={`h-100 px-4 sm:px-6 py-1 sm:py-2 inline-flex items-center justify-center text-sm border-0 bg-transparent outline-none  ${
                  language === 'javascript'
                    ? 'hover:no-underline'
                    : 'cursor-pointer'
                }`}
            ...
            <div className="z-0 -mx-3 overflow-y-scroll text-12">
              {sources[language] ? (
                <MDXRemote {...sources[language]} components={mdxComponents} />
              ) : null}
            </div>
          </div>
        </div>
